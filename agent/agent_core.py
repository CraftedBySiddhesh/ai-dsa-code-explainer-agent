...
class NotebookAgent:
    ...
    def run_one(self, problem_md: str):
        problem = groq_chat_json(
            prompts.EXTRACT_SYSTEM, prompts.EXTRACT_USER.format(md=problem_md[:140000]), max_tokens=2200
        )
        ...
        for _ in range(self.repair_rounds):
            v = groq_chat_json(prompts.VALIDATE_SYSTEM, prompts.VALIDATE_USER.format(md=md[:180000]), max_tokens=800)
            if v.get("error"):
                break
            if v.get("ok"):
                break
            ...
            md = groq_chat(
                ...
                prompts.REPAIR_USER.format(
                    problem_json=json.dumps(problem, ensure_ascii=False), issues=issues, md=md[:180000]
                ),
                temperature=0.2,
                max_tokens=3800,
            )
